...
public class DiscordSRV extends JavaPlugin {
    ...
    public void reloadAllowedMentions() {
        ...
        MessageAction.setDefaultMentions(
            config().getStringList("DiscordChatChannelAllowedMentions")
                .stream()
                .map(s -> {
                    try {
                        ...
                        DiscordSRV
                            .error("Unknown mention type \"" + s + "\" defined in DiscordChatChannelAllowedMentions");
                        return null;
                    }
                })
        ...
        DiscordSRV.debug(
            "Allowed chat mention types: " + MessageAction.getDefaultMentions()
                .stream()
                .map(Enum::name)
    ...
    public void reloadChannels() {
        synchronized (channels) {
            channels.clear();
            config().dget("Channels")
                .children()
                .forEach(
                    dynamic -> this.channels.put(
                        dynamic.key()
                            .convert()
                            .intoString(),
                        dynamic.convert()
    ...
    public void reloadRoleAliases() {
        synchronized (roleAliases) {
            roleAliases.clear();
            config().dget("DiscordChatChannelRoleAliases")
                .children()
                .forEach(
                    dynamic -> this.roleAliases.put(
                        dynamic.key()
                            .convert()
                            .intoString()
                            .toLowerCase(),
    ...
    public void reloadRegexes() {
        ...
        synchronized (webhookUsernameRegexes) {
            ...
            loadRegexesFromConfig(
                config().dget("Experiment_WebhookChatMessageUsernameFilters"),
    ...
    public String getMainChatChannel() {
        return channels.size() != 0 ? channels.keySet()
            .iterator()
            .next() : null;
    }
    ...
    public TextChannel getMainTextChannel() {
        ...
        String firstChannel = channels.values()
            .iterator()
            .next();
    ...
    public TextChannel getDestinationTextChannelForGameChannelName(String gameChannelName) {
        Map.Entry<String, String> entry = channels.entrySet()
            .stream()
            .filter(
                e -> e.getKey()
                    .equals(gameChannelName))
            .findFirst()
            .orElse(null);
        ...
        entry = channels.entrySet()
            .stream()
            .filter(
                e -> e.getKey()
                    .equalsIgnoreCase(gameChannelName))
            .findFirst()
            .orElse(null);
    ...
    public static void logThrowable(Throwable throwable, Consumer<String> logger) {
        ...
        for (String line : stringWriter.toString()
            .split("\n")) logger.accept(line);
    }
    ...
    public DiscordSRV() {
        ...
        try {
            ...
            getLogger()
                .info("If you fluently speak " + lang + " as well as English, see the GitHub repo to translate it!");
        }
        ...
        if (StringUtils.isNotBlank(forcedLanguage) && !forcedLanguage.equalsIgnoreCase("none")) {
            Arrays.stream(Language.values())
                .filter(
                    lang -> lang.getCode()
                        .equalsIgnoreCase(forcedLanguage)
                        || lang.getName()
                            .equalsIgnoreCase(forcedLanguage))
                .findFirst()
                .ifPresent(config::setLanguage);
        }
        ...
        try {
            ...
            for (String s : getGroupSynchronizables().keySet()) {
                permissions
                    .add(new org.bukkit.permissions.Permission("discordsrv.sync." + s, null, PermissionDefault.FALSE));
                permissions.add(
                    new org.bukkit.permissions.Permission("discordsrv.sync.deny." + s, null, PermissionDefault.FALSE));
            }
    ...
    public void onEnable() {
        if (++DebugUtil.initializationCount > 1) {
            ...
            PlayerUtil.getOnlinePlayers()
                ...
                .forEach(
                    player -> MessageUtil
                        .sendMessage(player, ChatColor.RED + LangUtil.InternalMessage.PLUGIN_RELOADED.toString()));
        }

        ConfigUtil.migrate();
        ConfigUtil.logMissingOptions();
        DiscordSRV.debug(
            "Language is " + config.getLanguage()
                .getName());
    ...
    public void disablePlugin() {
        ...
        if (pluginCommand != null && pluginCommand.getPlugin() == this) {
            try {
                Field owningPlugin = pluginCommand.getClass()
                    .getDeclaredField("owningPlugin");
    ...
    public void init() {
        if (Bukkit.getPluginManager()
            ...
            Plugin plugMan = Bukkit.getPluginManager()
                .getPlugin("PlugMan");
            try {
                List<String> ignoredPlugins = (List<String>) plugMan.getClass()
                    .getMethod("getIgnoredPlugins")
                    .invoke(plugMan);
        ...
        try {
            ...
            if (specialSourceFile.exists() && DigestUtils.md5Hex(FileUtils.readFileToByteArray(specialSourceFile))
                ...
                DiscordSRV.warning(
                    LangUtil.InternalMessage.ASM_WARNING.toString()
                        .replace(
                            "{specialsourcefolder}",
                            specialSourceFile.getParentFile()
                                .getPath()));
            }
        } catch (IOException e) {
            error(e);
        ...
        Dns dns = Dns.SYSTEM;
        try {
            List<InetAddress> fallbackDnsServers = new CopyOnWriteArrayList<>(
                Arrays.asList(
            ...
            dns = new Dns() {
                ...
                public List<InetAddress> lookup(@NotNull String host) throws UnknownHostException {
                    ...
                    // 0 = everything falls back (would only be useful when the system dns literally doesn't work &
                    // can't be fixed)
                    ...
                    if (max < 0 || (max > 0 && failedRequests < max)) {
                        try {
                            ...
                            failedRequests++;
                            DiscordSRV.error(
                                "System DNS FAILED to resolve hostname " + host
                                    + ", "
                                    + (max == 0 ? ""
                    ...
                    for (InetAddress dnsServer : fallbackDnsServers) {
                        try {
                            ...
                            if (query.responseCode != DnsMessage.RESPONSE_CODE.NO_ERROR) {
                                DiscordSRV.error(
                                    "DNS server " + dnsServer.getHostAddress()
                                        + " failed our DNS query for "
                                        + host
                                        + ": "
                                        + query.responseCode.name());
                            }
                            ...
                            DiscordSRV
                                .error("DNS server " + dnsServer.getHostAddress() + " failed to resolve " + host, e);
                        }
        ...
        OkHttpClient.Builder httpClientBuilder = new OkHttpClient.Builder().dispatcher(dispatcher)
        ...
        RestAction.setDefaultFailure(throwable -> {
            ...
            if (throwable instanceof HierarchyException) {
                ...
                DiscordSRV.error(
                    "DiscordSRV failed to perform an action because the bot is missing the "
                        + ((PermissionException) throwable).getPermission()
                            .name()
                        + " permission: "
                        + throwable.getMessage());
                ...
                if (cause instanceof InterruptedIOException && jda != null) {
                    ...
                    if (status == JDA.Status.SHUTDOWN || status == JDA.Status.SHUTTING_DOWN) {
                        // Ignore InterruptedIOException's during shutdown, we can't hold up the server from stopping
                        // forever,
                        // so some requests are cancelled during shutdown. Logging errors for those request failures
                        // isn't important.
                        return;
                    }
                }
                ...
                DiscordSRV.error(
                    "DiscordSRV encountered an unknown exception: " + throwable.getMessage()
                        + "\n"
            ...
            if (consoleChannel != null) {
                ...
                consoleAppender = new ChannelLoggingHandler(() -> {
                    TextChannel textChannel = DiscordSRV.getPlugin()
                        .getConsoleChannel();
                    return textChannel != null && textChannel.getGuild()
                        .getSelfMember()
                        .hasPermission(textChannel, Permission.MESSAGE_READ, Permission.MESSAGE_WRITE) ? textChannel
                            : null;
                }, config -> {
                    ...
                    Set<LogLevel> configuredLevels = config().getStringList("DiscordConsoleChannelLevels")
                        .stream()
                        ...
                        .map(s -> {
                            try {
                                ...
                                DiscordSRV.error(
                                    "Invalid console logging level '" + s
                                        + "', valid options are "
                    ...
                    config.addTransformer(logItem -> true, s -> MessageUtil.strip(DiscordUtil.aggressiveStrip(s))); // strip
                                                                                                                    // formatting
        ...
        // schedule slash commands to be updated later when plugins are ready (once the server had started up
        // completely)
        ...
        if (getMainTextChannel() != null && getConsoleChannel() != null
            && getMainTextChannel().getId()
                .equals(getConsoleChannel().getId()))
            ...
            try {
                ...
                do {
                    stringBuilder.append("\n")
                        .append("Caused by: ")
                        .append(
                            selected instanceof UnknownHostException ? "UnknownHostException"
                                : ExceptionUtils.getMessage(selected));
        ...
        Bukkit.getPluginManager()
            .registerEvents(accountLinkManager, this);
        ...
        for (String hookClassName : new String[] {
            ...
            "github.scarsz.discordsrv.hooks.chat.ChattyV3ChatHook", "github.scarsz.discordsrv.hooks.chat.FancyChatHook",
            "github.scarsz.discordsrv.hooks.chat.HerochatHook", "github.scarsz.discordsrv.hooks.chat.NChatHook", // nChat
                                                                                                                 // Hook
                                                                                                                 // needs
                                                                                                                 // to
                                                                                                                 // work
                                                                                                                 // before
                                                                                                                 // LegendChat
            "github.scarsz.discordsrv.hooks.chat.LegendChatHook", "github.scarsz.discordsrv.hooks.chat.LunaChatHook",
            "github.scarsz.discordsrv.hooks.chat.TownyChatHook", "github.scarsz.discordsrv.hooks.chat.VentureChatHook",
            // vanish plugins
            ...
            try {
                ...
                PluginHook pluginHook = (PluginHook) hookClass.getDeclaredConstructor()
                    .newInstance();
                if (pluginHook.isEnabled()) {
                    DiscordSRV.info(
                        LangUtil.InternalMessage.PLUGIN_HOOK_ENABLING.toString()
                            .replace(
                                "{plugin}",
                                pluginHook.getPlugin()
                                    .getName()));
                    Bukkit.getPluginManager()
                        .registerEvents(pluginHook, this);
                    try {
                        pluginHook.hook();
        ...
        if (pluginHooks.stream()
            .noneMatch(pluginHook -> pluginHook instanceof ChatHook)) {
            ...
            try {
                ...
                getServer().getPluginManager()
                    .registerEvents(new ModernPlayerChatListener(), this);
            ...
            boolean isWarning = warning != null && getServer().getWarningState()
                .printFor(warning); // check if the event has a nag

            Runnable registerLegacy = () -> getServer().getPluginManager()
            ...
            if (isWarning) {
                ...
                if (!configOption) {
                    ...
                    if (modernChatEventAvailable) {
                        warning(
                            "AsyncPlayerChatEvent will be registered because the UseModernPaperChatEvent config option is set to false");
                        warning(
                            "You should enable UseModernPaperChatEvent if your chat plugins have updated to using the new event");
                    } else {
        ...
        if (PluginUtil.pluginHookIsEnabled("PlaceholderAPI")) {
            try {
                DiscordSRV.info(
                    LangUtil.InternalMessage.PLUGIN_HOOK_ENABLING.toString()
                        .replace("{plugin}", "PlaceholderAPI"));
                SchedulerUtil.runTask(this, () -> {
                    try {
                        if (me.clip.placeholderapi.PlaceholderAPIPlugin.getInstance()
                            .getLocalExpansionManager()
                            .findExpansionByIdentifier("discordsrv")
                            .isPresent()) {
                        ...
                        getLogger()
                            .severe("Failed to hook into PlaceholderAPI, please check your PlaceholderAPI version");
                    }
                });
            } catch (Exception e) {
        ...
        if (isGroupRoleSynchronizationEnabled()) {
            int cycleTime = DiscordSRV.config()
                .getInt("GroupRoleSynchronizationCycleTime") * 20
                * 60;
            ...
            Bukkit.getPluginManager()
                .registerEvents(groupSynchronizationManager, this);
    ...
    public void onDisable() {
        ...
        if (Pattern.compile("%[^%]+%")
            .matcher(shutdownFormat)
            .find()) {
            shutdownFormat = PlaceholderUtil.replacePlaceholdersToDiscord(shutdownFormat);
        }

        final ThreadFactory threadFactory = new ThreadFactoryBuilder().setNameFormat("DiscordSRV - Shutdown")
            .build();
        ...
        try {
            ...
            executor.invokeAll(Collections.singletonList(() -> {
                ...
                if (!SchedulerUtil.isFolia()) { // not implemented yet on folia
                    for (BukkitWorker activeWorker : Bukkit.getScheduler()
                        .getActiveWorkers()) {
                        if (activeWorker.getOwner()
                            .equals(this)) {
                            List<String> stackTrace = Arrays.stream(
                                activeWorker.getThread()
                                    .getStackTrace())
                                .map(StackTraceElement::toString)
                                .collect(Collectors.toList());
                            warning(
                                "a DiscordSRV scheduler task still active during onDisable: " + stackTrace.remove(0));
                            debug(stackTrace);
                        }
                    }
                }
                ...
                if (jdaFilter != null) {
                    try {
                        ...
                        if (configField != null) {
                            ...
                            Field configField2 = config.getClass()
                ...
                // Clear JDA listeners
                if (jda != null) jda.getEventManager()
                    .getRegisteredListeners()
                    .forEach(
                        listener -> jda.getEventManager()
                            .unregister(listener));
                ...
                DiscordSRV.info(
                    LangUtil.InternalMessage.SHUTDOWN_COMPLETED.toString()
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label,
        String[] args) {
        Supplier<Boolean> handle = () -> commandManager.handle(
            sender,
            args[0],
            Arrays.stream(args)
                .skip(1)
        ...
        if (!isEnabled()) {
            ...
            if (invalidBotToken) {
                ...
                sender.sendMessage(
                    ChatColor.RED + "Please enter a valid token into your config.yml "
                        + "("
                        + ChatColor.GRAY
                ...
                sender.sendMessage(
                    ChatColor.RED + "Please check your server log "
                        + "("
                        + ChatColor.GRAY
                        + "/logs/latest.log"
                        + ChatColor.RED
                ...
                sender.sendMessage(
                    ChatColor.RED + "DiscordSRV is disabled, check your server log "
                        + "("
                        + ChatColor.GRAY
                        + "/logs/latest.log"
                        + ChatColor.RED
    ...
    public List<String> onTabComplete(@NotNull CommandSender sender, @NotNull Command bukkitCommand,
        ...
        if (command.equals("")) return new ArrayList<String>() {

            {
                for (Map.Entry<String, Method> command : getCommandManager().getCommands()
                    .entrySet())
                    if (GamePermissionUtil.hasPermission(
                        sender,
                        command.getValue()
                            .getAnnotation(github.scarsz.discordsrv.commands.Command.class)
                            .permission()))
                        add(command.getKey());
            }
        };
        if (commandArgs.length == 0) return new ArrayList<String>() {

            {
                for (Map.Entry<String, Method> commandPair : getCommandManager().getCommands()
                    .entrySet())
                    if (commandPair.getKey()
                        .toLowerCase()
                        .startsWith(command.toLowerCase()))
                        if (GamePermissionUtil.hasPermission(
                            sender,
                            commandPair.getValue()
                                .getAnnotation(github.scarsz.discordsrv.commands.Command.class)
                                .permission()))
    ...
    public void reloadCancellationDetector() {
        ...
        if (Debug.MINECRAFT_TO_DISCORD.isVisible()) {
            try {
                legacyCancellationDetector = new CancellationDetector<>(
                    this,
    ...
    public void processChatMessage(Player player, Component message, String channel, boolean cancelled,
        ...
        if (PluginUtil.pluginHookIsEnabled("mcMMO")) {
            if (player.hasMetadata("mcMMO: Player Data")) {
                ...
                if (usingAdminChat || usingPartyChat) {
                    debug(
                        Debug.MINECRAFT_TO_DISCORD,
                        "Not processing message because message was from " + (usingAdminChat ? "admin" : "party")
                            + " chat");
                    return;
                }
            }
        ...
        if (MessageUtil.strip(legacy)
            .startsWith(prefix) == blacklist) {
            debug(
                Debug.MINECRAFT_TO_DISCORD,
                "User " + player.getName()
                    + " sent a message but it was not delivered to Discord because "
                    + (blacklist ? "the message started with \"" + prefix : "the message didn't start with \"" + prefix)
        ...
        if (hasGoodGroup) userPrimaryGroup = userPrimaryGroup.substring(0, 1)
            .toUpperCase() + userPrimaryGroup.substring(1);

        boolean reserializer = DiscordSRV.config()
            .getBoolean("Experiment_MCDiscordReserializer_ToDiscord");
        ...
        if (!webhookMessageDelivery) {
            ...
            String discordMessagePattern = (hasGoodGroup ? LangUtil.Message.CHAT_TO_DISCORD.toString()
                : LangUtil.Message.CHAT_TO_DISCORD_NO_PRIMARY_GROUP.toString())
                    ...
                    .replace(
                        "%channelname%",
                        channel != null ? channel.substring(0, 1)
        ...
        GameChatMessagePostProcessEvent postEvent = api.callEvent(
            new GameChatMessagePostProcessEvent(channel, processedMessage, player, preEvent.isCancelled(), event));
        ...
        processedMessage = postEvent.getProcessedMessage(); // update message from event in case any listeners modified
                                                            // it
    ...
    private String processRegex(String discordMessage) {
        for (Map.Entry<Pattern, String> entry : getGameRegexes().entrySet()) {
            discordMessage = entry.getKey()
                .matcher(discordMessage)
                .replaceAll(entry.getValue());
            if (StringUtils.isBlank(discordMessage)) {
                DiscordSRV.debug(
                    Debug.MINECRAFT_TO_DISCORD,
                    "Not processing Minecraft message because it was cleared by a filter: " + entry.getKey()
                        .pattern());
                return null;
            }
        }
    ...
    public void broadcastMessageToMinecraftServer(String channel, String message, User author) {
        ...
        UUID authorLinkedUuid = DiscordSRV.getPlugin()
            .getAccountLinkManager()
            .getUuid(author.getId());
    ...
    public void broadcastMessageToMinecraftServer(String channel, Component message, User author) {
        ...
        if (chatHook == null || channel == null) {
            if (channel != null && !channel.equalsIgnoreCase("global")) return; // don't send messages for non-global
                                                                                // channels with no plugin hooks
            DiscordGuildMessagePreBroadcastEvent preBroadcastEvent = api.callEvent(
                new DiscordGuildMessagePreBroadcastEvent(author, channel, message, PlayerUtil.getOnlinePlayers()));
        ...
        if (DiscordSRV.config()
            .getBoolean("DiscordChatChannelBroadcastDiscordMessagesToConsole")) {
            DiscordSRV.info(
                LangUtil.InternalMessage.CHAT + ": "
                    + MessageUtil.strip(
                        MessageUtil.toLegacy(message)
                            .replace("»", ">")));
        }
    }
    ...
    public void sendJoinMessage(Player player, String joinMessage) {
        ...
        final String botAvatarUrl = DiscordUtil.getJda()
            .getSelfUser()
            .getEffectiveAvatarUrl();
        String botName = getMainGuild() != null ? getMainGuild().getSelfMember()
            .getEffectiveName()
            : DiscordUtil.getJda()
                .getSelfUser()
                .getName();
     ...
     *
     * @param player      the player
     * @param quitMessage the leave/quit message (that is usually provided by Bukkit's
     *                    {@link PlayerQuitEvent#getQuitMessage()})
     * @see #sendJoinMessage(Player, String)
     */
    ...
    public static Message translateMessage(MessageFormat messageFormat,
        ...
        Optional.ofNullable(messageFormat.getContent())
            .map(content -> translator.apply(content, true))
            .filter(StringUtils::isNotBlank)
            .ifPresent(messageBuilder::setContent);

        EmbedBuilder embedBuilder = new EmbedBuilder();
        embedBuilder.setAuthor(
            Optional.ofNullable(messageFormat.getAuthorName())
                .map(content -> translator.apply(content, false))
                .filter(StringUtils::isNotBlank)
                .orElse(null),
        ...
        embedBuilder.setThumbnail(
            Optional.ofNullable(messageFormat.getThumbnailUrl())
                .map(content -> translator.apply(content, true))
                .filter(StringUtils::isNotBlank)
                .orElse(null));
        embedBuilder.setImage(
            Optional.ofNullable(messageFormat.getImageUrl())
                .map(content -> translator.apply(content, true))
                .filter(StringUtils::isNotBlank)
                .orElse(null));
        embedBuilder.setDescription(
            Optional.ofNullable(messageFormat.getDescription())
                .map(content -> translator.apply(content, true))
                .filter(StringUtils::isNotBlank)
                .orElse(null));
        embedBuilder.setTitle(
            Optional.ofNullable(messageFormat.getTitle())
                .map(content -> translator.apply(content, false))
                .filter(StringUtils::isNotBlank)
                .orElse(null),
            Optional.ofNullable(messageFormat.getTitleUrl())
                .map(content -> translator.apply(content, true))
                .filter(StringUtils::isNotBlank)
                .orElse(null));
        embedBuilder.setFooter(
            ...
            Optional.ofNullable(messageFormat.getFooterIconUrl())
                ...
                .filter(StringUtils::isNotBlank)
                .orElse(null));
        if (messageFormat.getFields() != null) messageFormat.getFields()
            .forEach(
                field -> embedBuilder.addField(
                    translator.apply(field.getName(), true),
                    translator.apply(field.getValue(), true),
                    field.isInline()));
    ...
    private static String constructAvatarUrl(String username, UUID uuid, String texture) {
        ...
        if (StringUtils.isNotBlank(username) && offline) {
            ...
            // (can't do it by calling Bukkit.getOfflinePlayer(username).getUniqueId() because bukkit just returns the
            // offline-mode CraftPlayer)
        ...
        String avatarUrl = DiscordSRV.config()
            .getString("AvatarUrl");
        ...
        if (avatarUrl.contains("://crafatar.com/")) {
            avatarUrl = !offline ? defaultUrl : offlineUrl;
            DiscordSRV.config()
                .setRuntimeValue("AvatarUrl", avatarUrl);

            DiscordSRV.warning(
                "Your AvatarUrl config option uses crafatar.com, which no longer allows usage with Discord. An alternative provider will be used.");
            DiscordSRV.warning(
                "You should set your AvatarUrl (in config.yml) to an empty string (\"\") to get rid of this warning.");
        }

        if (offline && (avatarUrl.contains("{uuid}") || avatarUrl.contains("{uuid-nodashes}"))
            && !offlineUuidAvatarUrlNagged) {
            DiscordSRV.error(
                "Your AvatarUrl config option contains {uuid} or {uuid-nodashes} but this server is using offline UUIDs.");
        ...
        avatarUrl = avatarUrl.replace("{texture}", texture != null ? texture : "")
            .replace("{username}", username)
        ...
        DiscordSRV.debug(
            "Avatar url is for " + (offline ? "**offline** " : "") + "uuid: " + uuid + ". The texture is: " + texture);
    ...
    public List<Role> getSelectedRoles(Member member) {
        ...
        List<String> discordRolesSelection = DiscordSRV.config()
            .getStringList("DiscordChatChannelRolesSelection");
        // if we have a whitelist in the config
        if (DiscordSRV.config()
            .getBoolean("DiscordChatChannelRolesSelectionAsWhitelist")) {
            selectedRoles = member.getRoles()
                .stream()
                .filter(
                    role -> discordRolesSelection.contains(DiscordUtil.getRoleName(role))
    ...
    public Map<String, String> getGroupSynchronizables() {
        ...
        config.dget("GroupRoleSynchronizationGroupsAndRolesToSync")
            .children()
            .forEach(
                dynamic -> map.put(
                    dynamic.key()
    ...
    public Map<String, String> getCannedResponses() {
        ...
        config.dget("DiscordCannedResponses")
            .children()
            .forEach(dynamic -> {
                String trigger = dynamic.key()
                    .convert()
                    .intoString();
    ...
    public boolean isGroupRoleSynchronizationEnabled(boolean checkPermissions) {
        ...
        for (Map.Entry<String, String> entry : groupsAndRolesToSync.entrySet()) {
            ...
            if (!group.isEmpty()) {
                ...
                if (!(roleId.isEmpty() || roleId.replace("0", "")
                    .trim()
                    .isEmpty())) return true;
            }
        }
