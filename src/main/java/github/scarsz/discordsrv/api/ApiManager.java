 ...
 * </p>
 * <p>
 * DiscordSRV's API works exactly like Bukkit's.
 * </p>
 * <br>
 * <p>
 * To register your class to receive events:
 * </p>
 * <ol>
 * <li>Have a class with methods marked {@link Subscribe} that take the event to listen for as its <i>only</i>
 * parameter</li>
 * <li>Pass an instance of that class to {@link #subscribe(Object)}</li>
 * </ol>
 * <p>
 * {@link ListenerPriority} can optionally be set, defaulting to ListenerPriority.NORMAL
 * </p>
 *
...
public class ApiManager extends ListenerAdapter {
    ...
    public void subscribe(Object listener) {
        ...
        for (Method method : listener.getClass()
            .getMethods()) if (method.isAnnotationPresent(Subscribe.class)) methodsAnnotatedSubscribe++;
        if (methodsAnnotatedSubscribe == 0) throw new IllegalArgumentException(
            listener.getClass()
                .getName()
                + " attempted DiscordSRV API registration but no public methods inside of it were annotated @Subscribe (github.scarsz.discordsrv.api.Subscribe)");

        if (!listener.getClass()
            .getPackage()
            .getName()
            .contains("scarsz.discordsrv")) {
            DiscordSRV.info(
                LangUtil.InternalMessage.API_LISTENER_SUBSCRIBED.toString()
                    .replace(
                        "{listenername}",
                        listener.getClass()
                            .getName())
    ...
    public boolean unsubscribe(Object listener) {
        DiscordSRV.info(
            LangUtil.InternalMessage.API_LISTENER_UNSUBSCRIBED.toString()
                .replace(
                    "{listenername}",
                    listener.getClass()
                        .getName()));
    ...
    public <E extends Event> E callEvent(E event) {
        for (ListenerPriority listenerPriority : ListenerPriority.values()) {
            for (Object apiListener : apiListeners) {
                for (Method method : apiListener.getClass()
                    .getMethods()) {
                    ...
                    if (!method.getParameters()[0].getType()
                        .isAssignableFrom(event.getClass())) continue; // make sure this method wants this event

                    Subscribe subscribeAnnotation = method.getAnnotation(Subscribe.class);
                    if (subscribeAnnotation == null) continue;

                    if (subscribeAnnotation.priority() != listenerPriority) continue; // this priority isn't being
                                                                                      // called right now
    ...
    public void updateSlashCommands() {
        ...
        for (PluginSlashCommand pluginSlashCommand : commands) {
            String name = pluginSlashCommand.getCommandData()
                .getName();
            ...
            conflictingCommands++;
            if (pluginSlashCommand.getPriority()
                .ordinal()
                < conflictingCommand.getPriority()
                    .ordinal())
        ...
        for (Guild guild : DiscordSRV.getPlugin()
            ...
            GuildSlashCommandUpdateEvent event = DiscordSRV.api
        ...
        RestAction.allOf(guildCommandUpdateActions)
            .queue(all -> {
                ...
                long pluginCount = conflictResolvedCommands.values()
                    .stream()
                    .map(PluginSlashCommand::getPlugin)
                    .distinct()
                    .count();
                ...
                if (successful > 0) {
                    DiscordSRV.info(
                        "Successfully registered " + successful
                            + " slash commands ("
                            + finalConflictingCommands
                            + " conflicted) for "
                            + pluginCount
                            + " plugins in "
                            + registeredGuilds
                ...
                for (CommandRegistrationError error : errors) {
                    ...
                    if (!(exception instanceof ErrorResponseException)) {
                        DiscordSRV.warning(
                            "Unexpected error adding slash commands in server " + guild.getName()
                                + ": "
                                + exception.toString());
                        continue;
                    }
                    ...
                    if (response == ErrorResponse.MISSING_ACCESS) {
                        ...
                        DiscordSRV.warning(
                            "Failed to register slash commands in guild " + guild.getName()
                                + " ("
                                + guild.getId()
                                + ") due to error: "
                                + errorResponseException.getMeaning());
                    }
                }
                DiscordSRV
                    .error("Until this is fixed, plugin slash commands won't work properly in the specified guilds.");

                if (errors.stream()
                    .anyMatch(
                        r -> r.getException() instanceof ErrorResponseException
                            && ((ErrorResponseException) r.getException()).getErrorResponse()
                                == ErrorResponse.MISSING_ACCESS)) {
                    String invite = "https://scarsz.me/authorize#";
                    try {
                        invite += DiscordSRV.getPlugin()
                            .getJda()
                            .getSelfUser()
                            .getApplicationId();
                    ...
                    DiscordSRV.error(
                        "Re-authorize your bot at " + invite
                            + " to the respective guild to grant the applications.commands slash commands scope.");
                }
    ...
    /**
     * Event listener for JDA {@link SlashCommandEvent}. Automatically routes events to {@link SlashCommand}-annotated
     * methods on registered command providers.
     */
    @Override
     ...
     */
    private boolean handleSlashCommandEvent(SlashCommandProvider provider, PluginSlashCommand commandData,
        SlashCommandEvent event, SlashCommandPriority priority) {
        for (Method method : provider.getClass()
            .getMethods()) {
            for (SlashCommand slashCommand : method.getAnnotationsByType(SlashCommand.class)) {
                ...
                if (!GlobPattern.compile(slashCommand.path())
                    .matches(event.getCommandPath())) continue;
                if (method.getParameters().length != 1 || !method.getParameters()[0].getType()
                    .equals(SlashCommandEvent.class)) continue;
    ...
    public void requireIntent(GatewayIntent gatewayIntent) {
        if (DiscordSRV.getPlugin()
            .getJda() != null) throw new IllegalStateException("Intents must be required before JDA initializes");
    ...
    public void requireCacheFlag(CacheFlag cacheFlag) {
        if (DiscordSRV.getPlugin()
            .getJda() != null) throw new IllegalStateException("Cache flags must be required before JDA initializes");
