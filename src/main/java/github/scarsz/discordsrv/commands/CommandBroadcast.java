...
public class CommandBroadcast {
    ...
    public static void execute(CommandSender sender, String[] args) {
        ...
        if (args.length > 0 && args[0].startsWith("#")) {
            ...
            if (target == null) {
                List<TextChannel> mainGuildResults = DiscordSRV.getPlugin()
                    .getMainGuild()
                    .getTextChannelsByName(raw, true);
                if (mainGuildResults.size() >= 1) {
                    target = mainGuildResults.get(0);
                } else {
                    target = DiscordUtil.getJda()
                        .getGuilds()
                        .stream()
                        .flatMap(
                            guild -> guild.getTextChannelsByName(raw, true)
                                .stream())
            ...
            target = DiscordSRV.getPlugin()
                .getOptionalTextChannel("broadcasts");
            finalArgs = args;
        }

        if (finalArgs.length == 0) {
            MessageUtil
                .sendMessage(sender, ChatColor.RED + LangUtil.InternalMessage.NO_MESSAGE_GIVEN_TO_BROADCAST.toString());
        } else {
            ...
            if (DiscordSRV.config()
                .getBoolean("DiscordChatChannelTranslateMentions"))
                rawMessage = DiscordUtil.convertMentionsFromNames(
                    rawMessage,
            ...
            if (DiscordSRV.config()
                .getBoolean("Experiment_MCDiscordReserializer_InBroadcast")) {
