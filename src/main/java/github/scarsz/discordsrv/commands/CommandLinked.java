...
public class CommandLinked {
        ...
        if (args.length == 0) {
            if (!(sender instanceof Player)) {
                MessageUtil.sendMessage(
                    sender,
                    LangUtil.Message.LINKED_NOBODY_FOUND.toString()
                        .replace("%target%", "CONSOLE"));
                return;
            }

            String linkedId = DiscordSRV.getPlugin()
            ...
            if (hasLinkedAccount) {
                ...
                MessageUtil.sendMessage(
                    sender,
                    LangUtil.Message.LINKED_SUCCESS.toString()
                        .replace("%name%", name));
            } else {
            ...
            if (args.length == 1 && target.length() == 32 || target.length() == 36) {
                ...
                || (StringUtils.isNumeric(target) && target.length() >= 17 && target.length() <= 20)) {
                    ...
                    if (joinedTarget.contains("#") || (joinedTarget.length() >= 2 && joinedTarget.length() <= 32 + 5)) {
                        ...
                        Set<User> matches = DiscordUtil.getJda()
                            .getGuilds()
                            .stream()
                            .flatMap(
    ...
    static void notifyInterpret(CommandSender sender, String type) {
        MessageUtil
            .sendMessage(sender, String.format("%sInterpreted target as %s%s", ChatColor.AQUA, ChatColor.WHITE, type));
    }
    ...
    static void notifyPlayer(CommandSender sender, OfflinePlayer player) {
        MessageUtil.sendMessage(
            ...
            String.format(
                ...
                ChatColor.AQUA,
                ChatColor.WHITE,
                PrettyUtil.beautifyNickname(player)));
    }
