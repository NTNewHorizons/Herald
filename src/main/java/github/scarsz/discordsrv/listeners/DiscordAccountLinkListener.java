...
public class DiscordAccountLinkListener extends ListenerAdapter {
    ...
    public void onPrivateMessageReceived(PrivateMessageReceivedEvent event) {
        ...
        if (!DiscordSRV.config()
            .getBoolean("MinecraftDiscordAccountLinkedUsePM")) return;
    ...
    private final ErrorHandler ignoreFailedToDeleteMessage = new ErrorHandler().ignore(ErrorResponse.UNKNOWN_MESSAGE)
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        ...
        TextChannel linkChannel = DiscordSRV.getPlugin()
            .getDestinationTextChannelForGameChannelName("link");
        if (!event.getChannel()
        ...
        String reply = DiscordSRV.getPlugin()
            ...
            .process(
                ...
                event.getAuthor()
                    .getId());
        if (reply != null) {
            int deleteSeconds = DiscordSRV.config()
                .getIntElse("MinecraftDiscordAccountLinkedMessageDeleteSeconds", 0);
            RestAction<Message> repliedMessage = receivedMessage.reply(reply)
                .delay(deleteSeconds, TimeUnit.SECONDS);

            repliedMessage.queue(replyMessage -> {
                // delete the message after a delay if the config option is set
                if (deleteSeconds > 0) {
                    replyMessage.delete()
                        .queue(null, ignoreFailedToDeleteMessage);
                    receivedMessage.delete()
                        .queue(
                            null,
                            ignoreFailedToDeleteMessage.handle(
                                ErrorResponse.MISSING_PERMISSIONS,
                                e -> DiscordSRV.debug(
                                    Debug.ACCOUNT_LINKING,
                                    "Failed to delete " + receivedMessage.getAuthor()
                                        + "'s message in the link channel because of missing permissions.")));
                }
            });
    ...
    public void onGuildMemberJoin(GuildMemberJoinEvent event) {
        ...
        if (uuid != null) {
            ...
            if (roleToAdd == null || roleToAdd.getGuild()
                ...
                DiscordSRV.debug(
                    Debug.GROUP_SYNC,
                    "Not adding role to member upon guild join due to the guild being different! (" + roleToAdd
                        .getGuild() + " / " + member.getGuild() + ")");
            }
