...
public class DiscordConsoleListener extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) {
        ...
        if (DiscordSRV.config()
            .getBoolean("DiscordConsoleChannelBlockBots")
            && event.getAuthor()
                .isBot()) {
            DiscordSRV.debug(
                Debug.UNCATEGORIZED,
                "Received a message from a bot in the console channel, but DiscordConsoleChannelBlockBots is enabled");
            return;
        }

        boolean isWhitelist = DiscordSRV.config()
            .getBoolean("DiscordConsoleChannelBlacklistActsAsWhitelist");
        List<String> blacklistedCommands = DiscordSRV.config()
            .getStringList("DiscordConsoleChannelBlacklistedCommands");

        for (int i = 0; i < blacklistedCommands.size(); i++) blacklistedCommands.set(
            i,
            blacklistedCommands.get(i)
                .toLowerCase());

        String requestedCommand = event.getMessage()
            .getContentRaw()
            .trim()
            .split(" ")[0].toLowerCase();
            ...
            try {
                ...
                DiscordSRV.error(
                    LangUtil.InternalMessage.ERROR_LOGGING_CONSOLE_ACTION + " "
                        ...
                        + e.getMessage());
                if (DiscordSRV.config()
                    .getBoolean("CancelConsoleCommandIfLoggingFailed")) return;
            }
        }

        DiscordConsoleCommandPreProcessEvent consoleEvent = DiscordSRV.api.callEvent(
            new DiscordConsoleCommandPreProcessEvent(
                event,
                event.getMessage()
                    .getContentRaw(),
                true));
