...
public class PlayerAdvancementDoneListener implements Listener {
    ...
    private void runAsync(PlayerAdvancementDoneEvent event) {
        ...
        AchievementMessagePreProcessEvent preEvent = DiscordSRV.api.callEvent(
            new AchievementMessagePreProcessEvent(channelName, messageFormat, player, advancementTitle, event));
        if (preEvent.isCancelled()) {
            DiscordSRV.debug(
                Debug.MINECRAFT_TO_DISCORD,
                "AchievementMessagePreProcessEvent was cancelled, message send aborted");
            return;
        }
        ...
        BiFunction<String, Boolean, String> translator = (content, needsEscape) -> {
            ...
            content = content.replaceAll("%time%|%date%", TimeUtil.timeStamp())
                ...
                .replace(
                    "%achievement%",
                    MessageUtil
                        .strip(needsEscape ? DiscordUtil.escapeMarkdown(finalAchievementName) : finalAchievementName))
        ...
        AchievementMessagePostProcessEvent postEvent = DiscordSRV.api.callEvent(
        ...
        if (postEvent.isCancelled()) {
            DiscordSRV.debug(
                Debug.MINECRAFT_TO_DISCORD,
                "AchievementMessagePostProcessEvent was cancelled, message send aborted");
            return;
        }
        // Update from event in case any listeners modified parameters
    ...
    private boolean advancementIsHiddenInChat(Advancement advancement, World world) {
        ...
        if (ADVANCEMENT_DISPLAY_ANNOUNCE_CHAT_METHOD == null) {
            try {
                if (ADVANCEMENT_GET_DISPLAY_METHOD == null) ADVANCEMENT_GET_DISPLAY_METHOD = Arrays
                    .stream(
                ...
                DiscordSRV
                    .debug(Debug.MINECRAFT_TO_DISCORD, "Failed to find PlayerAdvancementDoneEvent#getDisplay method");
            }
        }
        ...
        if (ADVANCEMENT_DISPLAY_ANNOUNCE_CHAT_METHOD != null || advancementDisplay == null) {
            try {
                ...
                if (ADVANCEMENT_DISPLAY_ANNOUNCE_CHAT_METHOD == null) {
                    ADVANCEMENT_DISPLAY_ANNOUNCE_CHAT_METHOD = Arrays.stream(
                        craftAdvancementDisplay.getClass()
                            .getMethods())
                        .filter(
                ...
                boolean doesAnnounceToChat = (boolean) ADVANCEMENT_DISPLAY_ANNOUNCE_CHAT_METHOD
        ...
        if (advancementDisplay != null && !advancementDisplay.getClass()
            .getSimpleName()
            .equals("PaperAdvancementDisplay")
            && advancementDisplay instanceof org.bukkit.advancement.AdvancementDisplay) {
            ...
            try {
                ...
                DiscordSRV
                    .debug(Debug.MINECRAFT_TO_DISCORD, "Failed to check if advancement should be displayed: " + e);
            }
        }
        return false;
    }
    ...
    public static String getTitle(Advancement advancement) {
        return ADVANCEMENT_TITLE_CACHE.computeIfAbsent(advancement.getKey(), v -> {
            try {
                ...
                try {
                    Field advancementMessageField = advancementDisplay.getClass()
                        .getDeclaredField("a");
                    advancementMessageField.setAccessible(true);
                    Object advancementMessage = advancementMessageField.get(advancementDisplay);
                    Object advancementTitle = advancementMessage.getClass()
                        .getMethod("getString")
                        .invoke(advancementMessage);
                    DiscordSRV
                        .debug(Debug.MINECRAFT_TO_DISCORD, "Successfully retrieved advancement title from getString");
                    return (String) advancementTitle;
                } catch (Exception e) {
                    DiscordSRV.debug(
                        Debug.MINECRAFT_TO_DISCORD,
                        "Failed to get title of advancement using getString, trying JSON method");
                }

                Field titleComponentField = Arrays.stream(
                    advancementDisplay.getClass()
                        .getDeclaredFields())
                    .filter(field -> {
                        String simpleFieldName = field.getType()
                            .getSimpleName();
                        return simpleFieldName.equals("IChatBaseComponent")
                            || simpleFieldName.equals("IChatMutableComponent")
                            || simpleFieldName.equals("Component");
                    })
                    .findFirst()
                ...
                try {
                    method_getText = titleChatBaseComponent.getClass()
                        .getMethod("getText");
                ...
                if (method_getText != null) {
                    ...
                    DiscordSRV
                        .debug(Debug.MINECRAFT_TO_DISCORD, "Successfully retrieved advancement title from component");
                    if (StringUtils.isNotBlank(title)) return title;
                }

                Class<?> chatSerializerClass = Arrays.stream(
                    titleChatBaseComponent.getClass()
                        .getDeclaredClasses())
                    .filter(
                        clazz -> clazz.getSimpleName()
                            .equals("ChatSerializer"))
                    .findFirst()
                ...
                DiscordSRV.debug(
                    Debug.MINECRAFT_TO_DISCORD,
                    e,
                    "Failed to get title of advancement " + advancement.getKey()
                        .getKey() + ": " + e.getMessage());

                String rawAdvancementName = advancement.getKey()
                    .getKey();
                return Arrays.stream(
                    rawAdvancementName.substring(rawAdvancementName.lastIndexOf("/") + 1)
                        .toLowerCase()
                        .split("_"))
                    .map(
                        s -> s.substring(0, 1)
    ...
    private static Optional<Object> getAdvancementDisplayObject(Object handle)
        throws IllegalAccessException, InvocationTargetException {
        if (handle.getClass()
            .getSimpleName()
            .equals("AdvancementHolder")) {
            if (method_getAdvancementFromHolder == null) {
                method_getAdvancementFromHolder = Arrays.stream(
                    handle.getClass()
                        .getMethods())
                    .filter(
                        method -> method.getReturnType()
                            .getName()
                            .equals("net.minecraft.advancements.Advancement"))
                    .filter(method -> method.getParameterCount() == 0)
                    .findFirst()
                    .orElseThrow(() -> new RuntimeException("Failed to find Advancement from AdvancementHolder"));
            }
            ...
            if (method_getAdvancementFromHolder != null) {
                ...
                if (method_getAdvancementDisplay == null) {
                    method_getAdvancementDisplay = Arrays.stream(
                        holder.getClass()
                            .getMethods())
                        .filter(
                            method -> method.getReturnType()
                                ...
                                .equals("Optional"))
                        .filter(method -> {
                            String displayInfoReturnName = method.getGenericReturnType()
                                .getTypeName();
                            return displayInfoReturnName.contains("AdvancementDisplay")
                                || displayInfoReturnName.contains("DisplayInfo");
                        })
                        .findFirst()
                        .orElseThrow(
                ...
                Optional<Object> optionalAdvancementDisplay = (Optional<Object>) method_getAdvancementDisplay
                    .invoke(holder);
            ...
            if (method_getAdvancementDisplay == null) {
                method_getAdvancementDisplay = Arrays.stream(
                    handle.getClass()
                        .getMethods())
                    .filter(method -> {
                        String simpleReturnName = method.getReturnType()
                            .getSimpleName();
                    ...
                    .findFirst()
                    .orElseThrow(
                        () -> new RuntimeException("Failed to find AdvancementDisplay getter for advancement handle"));
            }
