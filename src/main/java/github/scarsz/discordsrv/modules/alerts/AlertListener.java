...
public class AlertListener implements Listener, EventListener {
    ...
    private static final Pattern VALID_CLASS_NAME_PATTERN = Pattern
    ...
    public AlertListener() {
        ...
        }, (listener, event) -> runAlertsForEvent(event), EventPriority.MONITOR, DiscordSRV.getPlugin(), false);
        reloadAlerts();
    }
    ...
    private void addListener(HandlerList handlerList) {
        for (Class<?> blacklistedClass : BLACKLISTED_CLASSES) {
            try {
                HandlerList list = (HandlerList) blacklistedClass.getMethod("getHandlerList")
                    .invoke(null);
                if (handlerList == list) {
                    DiscordSRV.debug(
                        Debug.ALERTS,
                        "Skipping registering HandlerList for " + blacklistedClass.getName() + " for alerts");
                    return;
                }
            } catch (NoSuchMethodException | InvocationTargetException | IllegalAccessException e) {
                DiscordSRV.debug(
                    Debug.ALERTS,
                    "Failed to check if HandlerList was for " + blacklistedClass.getName() + ": " + e.toString());
            }
        }
        for (StackTraceElement stackTraceElement : Thread.currentThread()
            .getStackTrace()) {
            String match = BLACKLISTED_CLASS_NAMES.stream()
                .filter(
                    className -> stackTraceElement.getClassName()
                        .equals(className))
                .findAny()
                .orElse(null);
            if (match != null && stackTraceElement.getMethodName()
                .equals("<clinit>")) {
                DiscordSRV.debug(
                    Debug.ALERTS,
                    "Skipping registering HandlerList for " + match + " for alerts (during event init)");
                return;
            }
        }
        if (Arrays.stream(handlerList.getRegisteredListeners())
            .noneMatch(listener::equals)) handlerList.register(listener);
    }
    ...
    public void reloadAlerts() {
        ...
        alerts.clear();
        Optional<List<Map<?, ?>>> optionalAlerts = DiscordSRV.config()
            .getOptional("Alerts");
        if (registered) unregister();

        if (!optionalAlerts.isPresent() || optionalAlerts.get()
            .isEmpty()) {
            return;
        }
        ...
        if (!simpleClassNames.isEmpty()) {
            DiscordSRV.warning(
                "Some alerts are using simple class names as triggers (instead of fully-classified class names including the package name), server performance may be effected.");
            DiscordSRV.warning("Support for simple class names will be removed in a future release of DiscordSRV");
            DiscordSRV.warning(
                "The following triggers are causing this notification: " + String.join(", ", simpleClassNames));
    ...
    private void runAlertsForEvent(Object event) {
        ...
        if (!active) {
            if (event instanceof Event) {
                // remove us from HandlerLists that we don't need (we can do this here, since we have the full class
                // name)
        ...
        for (int i = 0; i < alerts.size(); i++) {
            ...
            if (asyncDynamic.isPresent()) {
                if (asyncDynamic.convert()
                    .intoString()
                    .equalsIgnoreCase("false")
                    || asyncDynamic.convert()
                        .intoString()
                        .equalsIgnoreCase("no")) {
                    async = false;
                }
            ...
            if (async) {
                int alertIndex = i;
                SchedulerUtil
                    .runTaskAsynchronously(DiscordSRV.getPlugin(), () -> process(event, alert, triggers, alertIndex));
            } else {
    ...
    private String getEventClassName(Object event) {
        String className = event instanceof String ? (String) event
            : event.getClass()
                .getName();
    ...
    private void process(Object event, Dynamic alert, Set<String> triggers, int alertIndex) {
        ...
        if (player == null) {
            ...
            try {
                Method getPlayerMethod = event.getClass()
                    .getMethod("getPlayer");
                if (getPlayerMethod.getReturnType()
                    .equals(Player.class)) {
        ...
        MessageFormat messageFormat = DiscordSRV.getPlugin()
            .getMessageFromConfiguration("Alerts." + alertIndex);

        String eventClassName = getEventClassName(event);
        String eventName = getEventName(event);
        for (String trigger : triggers) {
            if (trigger.startsWith("/")) {
                if (StringUtils.isBlank(command) || !command.toLowerCase()
                    .split("\\s+|$", 2)[0].equals(trigger.substring(1))) continue;
            } else {
            ...
            if (event instanceof Cancellable && ((Cancellable) event).isCancelled()) {
                ...
                boolean ignoreCancelled = ignoreCancelledDynamic.isPresent() ? ignoreCancelledDynamic.as(Boolean.class)
                    : true;
                if (ignoreCancelled) {
                    DiscordSRV
                        .debug(Debug.ALERTS, "Not running alert for event " + eventName + ": event was cancelled");
                    return;
                }
            }
            ...
            if (textChannelsDynamic == null) {
                DiscordSRV.debug(
                    Debug.ALERTS,
                    "Not running alert for trigger " + trigger + ": no target channel was defined");
                return;
            }
            ...
            Set<TextChannel> textChannels = channelResolver.apply(s -> {
                TextChannel target = DiscordSRV.getPlugin()
                    .getDestinationTextChannelForGameChannelName(s);
                return Collections.singleton(target);
            });
            if (textChannels.isEmpty()) {
                textChannels.addAll(
                    channelResolver.apply(
                        s -> DiscordUtil.getJda()
                            .getTextChannelsByName(s, false)));
            }
            if (textChannels.isEmpty()) {
                textChannels.addAll(
                    channelResolver.apply(
                        s -> NumberUtils.isDigits(s) ? Collections.singleton(
                            DiscordUtil.getJda()
            ...
            if (textChannels.size() == 0) {
                DiscordSRV.debug(
                    Debug.ALERTS,
                    "Not running alert for trigger " + trigger
                        + ": no target channel was defined/found (channels: "
                        + channels
                        + ")");
                return;
            }
            ...
            for (TextChannel textChannel : textChannels) {
                ...
                if (conditionsDynamic.isPresent()) {
                    Iterator<Dynamic> conditions = conditionsDynamic.children()
                        .iterator();
                    while (conditions.hasNext()) {
                        Dynamic dynamic = conditions.next();
                        String expression = dynamic.convert()
                            .intoString();
                        try {
                            Boolean value = new SpELExpressionBuilder(expression).withPluginVariables()
                            ...
                            DiscordSRV.error(
                                "Error while parsing expression \"" + expression
                                    + "\" for trigger \""
                                    + trigger
                                    + "\" -> "
                                    + e.getMessage());
                ...
                BiFunction<String, Boolean, String> translator = (content, needsEscape) -> {
                    ...
                    content = NamedValueFormatter.format(content, key -> {
                        switch (key) {
                            ...
                            case "displayname":
                                return finalPlayer != null ? MessageUtil.strip(
                                    needsEscape ? DiscordUtil.escapeMarkdown(finalPlayer.getDisplayName())
                                        : finalPlayer.getDisplayName())
                                    : "";
                            case "world":
                                return finalPlayer != null ? finalPlayer.getWorld()
                                    .getName() : "";
                            case "embedavatarurl":
                                return finalPlayer != null ? DiscordSRV.getAvatarUrl(finalPlayer)
                                    : DiscordUtil.getJda()
                                        .getSelfUser()
                ...
                if (message == null) {
                    DiscordSRV
                        .debug(Debug.ALERTS, "Not sending alert because it is configured to have no message content");
                    return;
                }
