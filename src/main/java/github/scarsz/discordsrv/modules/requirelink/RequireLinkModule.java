...
public class RequireLinkModule implements Listener {
    ...
    private void check(String eventType, EventPriority priority, String playerName, UUID playerUuid, String ip,
        BiConsumer<String, String> disallow) {
        if (!isEnabled()) return;
        if (!eventType.equals(
            DiscordSRV.config()
                .getString("Require linked account to play.Listener event")))
            return;

        String requestedPriority = DiscordSRV.config()
            .getString("Require linked account to play.Listener priority");
        EventPriority targetPriority = Arrays.stream(EventPriority.values())
            .filter(
                p -> p.name()
                    .equalsIgnoreCase(requestedPriority))
            .findFirst()
            .orElse(EventPriority.LOWEST);
        ...
        try {
            if (getBypassNames().contains(playerName)) {
                DiscordSRV.debug(
                    Debug.REQUIRE_LINK,
                    "Player " + playerName + " is on the bypass list, bypassing linking checks");
                return;
            }

            if (checkWhitelist()) {
                boolean whitelisted = Bukkit.getServer()
                    .getWhitelistedPlayers()
                    .stream()
                    .map(OfflinePlayer::getUniqueId)
                    .anyMatch(u -> u.equals(playerUuid));
                if (whitelisted) {
                    DiscordSRV.debug(
                        Debug.REQUIRE_LINK,
                        "Player " + playerName + " is bypassing link requirement, player is whitelisted");
                    return;
                }
            }
            ...
            if (!checkBannedPlayers() || onlyCheckBannedPlayers) {
                ...
                if (Bukkit.getServer()
                    ...
                    if (!onlyCheckBannedPlayers) {
                        DiscordSRV
                            .debug(Debug.REQUIRE_LINK, "Player " + playerName + " is banned, skipping linked check");
                        return;
                    }
                    banned = true;
                }
                if (!banned && Bukkit.getServer()
                    .getIPBans()
                    .stream()
                    .anyMatch(ip::equals)) {
                    if (!onlyCheckBannedPlayers) {
                        DiscordSRV.debug(
                            Debug.REQUIRE_LINK,
                            "Player " + playerName + " connecting with banned IP " + ip + ", skipping linked check");
                        return;
                    }
                    banned = true;
                }
                if (onlyCheckBannedPlayers && !banned) {
                    DiscordSRV.debug(
                        Debug.REQUIRE_LINK,
                        "Player " + playerName
                            + " is bypassing link requirement because \"Only check banned players\" is enabled");
                    return;
                }
            }
            ...
            if (discordId == null) {
                ...
                DiscordSRV.debug(
                    Debug.REQUIRE_LINK,
                    "Player " + playerName + " is NOT linked to a Discord account, denying login");
                disallow.accept(
                    KICK_REASON_NOT_ALLOWED,
                    MessageUtil.translateLegacy(
                        DiscordSRV.config()
                            .getString("Require linked account to play.Not linked message"))
                        .replace("{BOT}", botName)
                        .replace("{CODE}", code)
            ...
            Dynamic mustBeInDiscordServerOption = DiscordSRV.config()
            ...
            if (mustBeInDiscordServerOption.is(Boolean.class)) {
                ...
                if (mustBeInDiscordServerOption.isList()) {
                    mustBeInDiscordServerOption.children()
                        .forEach(dynamic -> targets.add(dynamic.toString()));
                } else {
                    targets.add(
                ...
                for (String guildId : targets) {
                    try {
                        Guild guild = DiscordUtil.getJda()
                        ...
                        if (guild != null) {
                            ...
                            DiscordSRV.debug(
                                Debug.REQUIRE_LINK,
                                "Failed to get Discord server by ID " + guildId + ": bot is not in server");
                        }
            ...
            if (isSubRoleRequired() && !subRoleIds.isEmpty()) {
                ...
                for (String subRoleId : subRoleIds) {
                    ...
                    Member member = role.getGuild()
                        .getMemberById(discordId);
                    if (member != null && member.getRoles()
                        .contains(role)) {
                        matches++;
                    }
                }
                ...
                if (failedRoleIds == subRoleIds.size()) {
                    DiscordSRV.error(
                        "Tried to authenticate " + playerName
                            + " but no valid subscriber role IDs are found and thats a requirement; login will be denied until this is fixed.");
                    disallow.accept(
                        KICK_REASON_CONFIG_ERROR,
                        MessageUtil.translateLegacy(getFailedToFindRoleKickMessage()));
                    return;
                }
                ...
                if (getAllSubRolesRequired() ? matches < subRoleIds.size() : matches == 0) {
                    DiscordSRV.debug(
                        Debug.REQUIRE_LINK,
                        "Player " + playerName + " does NOT match subscriber role requirements, denying login");
                    disallow
    ...
    public void noticePlayerUnlink(Player player) {
        ...
        if (checkWhitelist()) {
            boolean whitelisted = Bukkit.getServer()
                ...
                .stream()
                .map(OfflinePlayer::getUniqueId)
                .anyMatch(u -> u.equals(player.getUniqueId()));
            if (whitelisted) {
                DiscordSRV.debug(
                    Debug.REQUIRE_LINK,
                    "Player " + player.getName() + " is bypassing link requirement, player is whitelisted");
                return;
            }
        }
        String ip = player.getAddress()
            .getAddress()
            .getHostAddress();
        if (onlyCheckBannedPlayers() && !Bukkit.getServer()
            .getBannedPlayers()
            .stream()
            .anyMatch(
                p -> p.getUniqueId()
        ...
        SchedulerUtil.runTaskForPlayer(
    ...
    private boolean checkBannedPlayers() {
        return DiscordSRV.config()
            .getBoolean("Require linked account to play.Check banned players");
    }
    ...
    private boolean getAllSubRolesRequired() {
        return DiscordSRV.config()
            .getBoolean("Require linked account to play.Subscriber role.Require all of the listed roles");
    }

    private boolean isEnabled() {
        return DiscordSRV.config()
            .getBoolean("Require linked account to play.Enabled");
    }
    ...
    private String getFailedToFindRoleKickMessage() {
        return DiscordSRV.config()
            .getString("Require linked account to play.Messages.Failed to find subscriber role");
    }

    private String getSubscriberRoleKickMessage() {
        return DiscordSRV.config()
            .getString("Require linked account to play.Subscriber role.Kick message");
    }

    private String getUnknownFailureKickMessage() {
        return DiscordSRV.config()
            .getString("Require linked account to play.Messages.Failed for unknown reason");
    }

    private String getUnlinkedKickMessage() {
        return DiscordSRV.config()
            .getString("Require linked account to play.Messages.Kicked for unlinking");
    }

    @EventHandler(priority = EventPriority.LOWEST)
    public void onEventLowest(AsyncPlayerPreLoginEvent event) {
        if (!event.getLoginResult()
            .equals(AsyncPlayerPreLoginEvent.Result.ALLOWED)) {
            DiscordSRV.debug(
                Debug.REQUIRE_LINK,
                "PlayerLoginEvent event result for " + event.getName() + " = " + event.getLoginResult() + ", skipping");
            return;
        }
        check(
            event.getClass()
            ...
            event.getAddress()
                .getHostAddress(),
            (result, message) -> event.disallow(AsyncPlayerPreLoginEvent.Result.valueOf(result), message));
    }
    ...
    public void onEventLowest(PlayerLoginEvent event) {
        if (!event.getResult()
            .equals(PlayerLoginEvent.Result.ALLOWED)) {
            DiscordSRV.debug(
                Debug.REQUIRE_LINK,
                "PlayerLoginEvent event result for " + event.getPlayer()
                    .getName() + " = " + event.getResult() + ", skipping");
            return;
        }
        check(
            event.getClass()
            ...
            event.getAddress()
                .getHostAddress(),
            (result, message) -> event.disallow(PlayerLoginEvent.Result.valueOf(result), message));
    }
