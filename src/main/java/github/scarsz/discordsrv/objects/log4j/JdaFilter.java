        ...
        switch (level.name()) {
            ...
            case "ERROR":
                if (message.contains("Requester timed out while executing a request")) {
                    DiscordSRV.error(
                        "[JDA] " + message
                            + ". This is either a issue on Discord's end (https://discordstatus.com) or with your server's connection");
                ...
                if (message.contains("There was an I/O error while executing a REST request: null")
                    && DiscordSRV.shuttingDown) {
                    // Error executing request while shutting down (interrupted exceptions)
                    return Result.DENY;
                }
                ...
                break;
            default:
                if (Debug.JDA.isVisible()) DiscordSRV.debug("[JDA] " + message);
        }
