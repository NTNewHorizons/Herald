...
public class CommandManager {
            ...
            for (Method method : clazz.getMethods()) {
                ...
                if (method.getParameters().length != 2) {
                    DiscordSRV.debug(
                        "Method " + method.toGenericString()
                            .replace("public static void ", "") + " annotated as command but parameters count != 2");
                    continue;
                }
                if (method.getParameters()[0].getType() != CommandSender.class
                    && method.getParameters()[0].getType() != Player.class) {
                    DiscordSRV.debug(
                        "Method " + method.toGenericString()
                            .replace("public static void ", "")
                            + " annotated as command but parameter 1's type != CommandSender || Player");
                    continue;
                }
                if (method.getParameters()[1].getType() != String[].class) {
                    DiscordSRV.debug(
                        "Method " + method.toGenericString()
                            .replace("public static void ", "")
                            + " annotated as command but parameter 2's type != String[]");
                    continue;
                }
    ...
    public boolean handle(CommandSender sender, String command, String[] args) {
        if (command == null) {
            OfflinePlayer offlinePlayer = sender instanceof Player
                ? Bukkit.getOfflinePlayer(((Player) sender).getUniqueId())
                : null;
            String message = PlaceholderUtil
                .replacePlaceholders(LangUtil.Message.DISCORD_COMMAND.toString(), offlinePlayer)
                .replace(
                    "{INVITE}",
                    DiscordSRV.config()
        ...
        if (commands.containsKey(command.toLowerCase())) {
            try {
                ...
                if (commandMethod.getParameters()[0].getType() == Player.class && !(sender instanceof Player)) {
                    MessageUtil
                        .sendMessage(sender, ChatColor.RED + LangUtil.InternalMessage.PLAYER_ONLY_COMMAND.toString());
                    return true;
                }
