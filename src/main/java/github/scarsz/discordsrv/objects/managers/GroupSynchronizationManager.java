...
public class GroupSynchronizationManager extends ListenerAdapter implements Listener {
    ...
    public void resync(SyncDirection direction, SyncCause cause) {
        ...
        if (DiscordSRV.config()
            .getBoolean("GroupRoleSynchronizationCycleCompletely")) {
            ...
            DiscordUtil.getJda()
                ...
                .stream()
                .flatMap(
                    guild -> guild.getMembers()
                        .stream())
                .map(
                    member -> DiscordSRV.getPlugin()
    ...
    public void resync(User user, SyncDirection direction, SyncCause cause) {
        UUID uuid = DiscordSRV.getPlugin()
            .getAccountLinkManager()
            .getUuid(user.getId());
        if (uuid == null) {
            DiscordSRV.debug(
                Debug.GROUP_SYNC,
                "Tried to sync groups for " + user + " but their Discord account is not linked to a MC account");
            return;
        }
    ...
    public void resync(OfflinePlayer player, SyncDirection direction, boolean addLinkedRole, SyncCause cause) {
        ...
        if (DiscordSRV.getPlugin()
            ...
            DiscordSRV.debug(
                Debug.GROUP_SYNC,
                "Tried to sync groups for player " + player.getName()
        ...
        if (discordId == null) {
            DiscordSRV.debug(
                Debug.GROUP_SYNC,
                "Tried to sync groups for player " + player.getName()
                    + " but their MC account is not linked to a Discord account. Synchronization cause: "
                    + cause);
            return;
        }
        ...
        synchronizationSummary.add(
            "Group synchronization (#" + id
                + ") "
                + direction
                + " for "
                + "{"
        ...
        boolean oneWaySynchronisation = DiscordSRV.config()
            .getBoolean("GroupRoleSynchronizationOneWay");
        boolean minecraftIsStrictlyAuthoritative = oneWaySynchronisation && DiscordSRV.config()
            .getBoolean("GroupRoleSynchronizationMinecraftIsAuthoritative");
        boolean discordIsStrictlyAuthoritative = oneWaySynchronisation && !DiscordSRV.config()
            .getBoolean("GroupRoleSynchronizationMinecraftIsAuthoritative");
        if (oneWaySynchronisation) synchronizationSummary.add(
        ...
        for (Map.Entry<String, String> entry : DiscordSRV.getPlugin()
            ...
            if (guild.getMember(user) != null) membersNotInGuild.remove(user.getId()); // is in cache, so is in the
                                                                                       // server too
            if (membersNotInGuild.contains(user.getId())) {
                synchronizationSummary.add(
                    "Tried to sync role " + role
            ...
            Member member;
            try {
                member = guild.retrieveMember(user, false)
                    .complete();
            ...
            boolean primaryGroupOnly = DiscordSRV.config()
                .getBoolean("GroupRoleSynchronizationPrimaryGroupOnly");
            ...
            try {
                ...
                if (DiscordSRV.config()
                    .getBoolean("GroupRoleSynchronizationEnableDenyPermission")
            ...
            boolean hasRole = member != null && member.getRoles()
                .contains(role);
            ...
            boolean minecraftIsAuthoritative = minecraftIsStrictlyAuthoritative
                || (!roleIsManaged && !discordIsStrictlyAuthoritative
                    && (direction == SyncDirection.AUTHORITATIVE ? DiscordSRV.config()
                        .getBoolean("GroupRoleSynchronizationMinecraftIsAuthoritative")
        ...
        if (!groupsGrantedByPermission.isEmpty()) {
            synchronizationSummary.add(
                "The following groups were granted due to having the " + "discordsrv.sync.<group name> permission(s): "
                    + String.join(" | ", groupsGrantedByPermission));
        }
        if (!groupsDeniedByPermission.isEmpty()) {
            synchronizationSummary.add(
                "The player does not have the following groups due to having the "
        ...
        if (addLinkedRole) {
            try {
                Role role = DiscordUtil.resolveRole(
                    DiscordSRV.config()
                        .getString("MinecraftDiscordAccountLinkedRoleNameToAddUserTo"));
                if (role != null) {
                    ...
                    DiscordSRV
                        .debug(Debug.GROUP_SYNC, "Couldn't add user to null (\"linked\") role to " + player.getName());
                }
            } catch (Throwable t) {
                DiscordSRV.debug(
                    Debug.GROUP_SYNC,
                    "Couldn't add \"linked\" role to " + player.getName()
                        + " due to exception: "
        ...
        for (Map.Entry<Guild, Map<String, Set<Role>>> guildEntry : roleChanges.entrySet()) {
            ...
            Set<Role> add = guildEntry.getValue()
                .getOrDefault("add", Collections.emptySet());
            Set<Role> remove = guildEntry.getValue()
                .getOrDefault("remove", Collections.emptySet());
            ...
            if (!selfMember.canInteract(member)) {
                synchronizationSummary
                    .add(
                        "Synchronization failed for " + member
                            + ": can't interact with member"
                ...
                synchronizationSummary.add(
                    "Bot's top role in " + guild
                        + ": "
                        + (selfMember.getRoles()
                            .isEmpty() ? "bot has no roles"
                                : selfMember.getRoles()
                                    .get(0) + " ("
                                    + selfMember.getRoles()
                                        .get(0)
            ...
            while (addIterator.hasNext()) {
                ...
                if (!selfMember.canInteract(role)) {
                    synchronizationSummary.add(
                        "Synchronization for role " + role
                            + " (add) in "
                            + guild
                            + " failed: can't interact with role ("
                            + role.getPosition()
                            + ")");
                    addIterator.remove();
            ...
            guild.modifyMemberRoles(member, add, remove)
                .reason("DiscordSRV synchronization")
                .queue(
                    v -> DiscordSRV.debug(
                        Debug.GROUP_SYNC,
                        "Synchronization #" + id
    ...
    public void resyncEveryone(SyncDirection direction, SyncCause cause) {
        ...
        Map<String, UUID> linkedDiscords = DiscordSRV.getPlugin()
            .getAccountLinkManager()
            .getManyUuids(
                DiscordUtil.getJda()
                    .getGuilds()
    ...
    public void removeSynchronizables(OfflinePlayer player) {
        if (!DiscordSRV.config()
            ...
            for (String group : DiscordSRV.getPlugin()
                .getGroupSynchronizables()
                .keySet()) {
            ...
            DiscordSRV.debug(
                Debug.GROUP_SYNC,
                player.getName() + " removed from their groups (Discord is authoritative). succeeded: "
                    + success
                    + ", failed: "
                    + fail);
        } else {
    ...
    public void removeSynchronizedRoles(OfflinePlayer player) {
        ...
        if (user != null) {
            ...
            DiscordSRV.getPlugin()
                ...
                .filter(Objects::nonNull)
                .forEach(
                    role -> roles.computeIfAbsent(role.getGuild(), guild -> new HashSet<>())
                        .add(role));

            try {
                // remove user from linked role
                String linkRole = DiscordSRV.config()
                    .getString("MinecraftDiscordAccountLinkedRoleNameToAddUserTo");
                Role role = StringUtils.isNotBlank(linkRole) ? DiscordUtil.resolveRole(linkRole) : null;
                if (role != null) {
                    roles.computeIfAbsent(role.getGuild(), guild -> new HashSet<>())
                        .add(role);
                } else {
                ...
                DiscordSRV.debug(
                    Debug.GROUP_SYNC,
                    "Failed to remove \"linked\" role from " + player.getName()
                        + " during unlink: "
                        + ExceptionUtils.getMessage(t));
            }
    ...
    public void onGuildMemberJoin(@NotNull GuildMemberJoinEvent event) {
        membersNotInGuilds.computeIfAbsent(
            event.getGuild()
                .getId(),
            key -> new HashSet<>())
            .remove(
    ...
    private void onGuildMemberRolesChanged(String type, Member member, List<Role> roles) {
        if (!DiscordSRV.getPlugin()
            .isGroupRoleSynchronizationEnabled()) return;
        List<Role> checkRoles = new ArrayList<>(roles);
        Collection<String> validRoleIds = DiscordSRV.getPlugin()
            .getGroupSynchronizables()
            .values();
        ...
        if (justModifiedRoles.containsKey(member)) {
            ...
            if (entry.getKey()
                .equals(member.getGuild())) {
                Set<Role> recentlyChanged = entry.getValue()
    ...
    private final List<Pattern> patterns = Arrays.asList(
        // GroupManager
        Pattern
            .compile("/?manu(?:add(?:sub)?|del(?:sub)?|promote|demote) " + userRegex + ".*", Pattern.CASE_INSENSITIVE),
        // PermissionsEx
        Pattern.compile(
            "/?pex user " + userRegex + " group(?: timed)? (?:add)|(?:set)|(?:remove) .*",
            Pattern.CASE_INSENSITIVE),
        // zPermissions
        Pattern.compile(
            "/?permissions player " + userRegex + " (?:(?:setgroup)|(?:addgroup)|(?:removegroup)).*",
            Pattern.CASE_INSENSITIVE),
    ...
    public Permission getPermissions() {
        if (permission != null) {
            ...
            try {
                ...
                if (!warnedAboutMissingVault && DiscordSRV.getPlugin()
                    .isGroupRoleSynchronizationEnabled(false)) {
                    DiscordSRV.error(
