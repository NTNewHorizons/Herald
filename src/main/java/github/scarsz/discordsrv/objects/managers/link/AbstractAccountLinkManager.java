...
public abstract class AbstractAccountLinkManager implements AccountLinkManager {
    ...
    protected void ensureOffThread(boolean single) {
        ...
        if (apiUser.startsWith("github.scarsz.discordsrv")) {
            DiscordSRV
                .warning("Linked account data requested on main thread, please report this to DiscordSRV: " + apiUser);
        ...
        DiscordSRV.warning(
            "API user " + apiUser
                + " requested linked account information on the main thread while MySQL is enabled in DiscordSRV's settings");
        if (single) {
            DiscordSRV.warning(
                "Requesting data for offline players on the main thread will lead to an exception in the future, if being on the main thread is explicitly required use getDiscordIdBypassCache / getUuidBypassCache");
        } else {
            DiscordSRV.warning(
                "Managing / Requesting bulk linked account data on the main thread will lead to an exception in the future");
        }
    ...
    protected void afterLink(String discordId, UUID uuid) {
        ...
        for (String command : DiscordSRV.config()
            .getStringList("MinecraftDiscordAccountLinkedConsoleCommands")) {
            ...
            command = command
                ...
                .replace(
                    ...
                    PrettyUtil.beautifyNickname(offlinePlayer, "[Unknown Player]", false))
            ...
            SchedulerUtil
                .runTask(DiscordSRV.getPlugin(), () -> Bukkit.dispatchCommand(Bukkit.getConsoleSender(), finalCommand));
        }

        // group sync using the authoritative side
        if (DiscordSRV.config()
            .getBoolean("GroupRoleSynchronizationOnLink")
            ...
            try {
                ...
                if (roleToAdd != null) {
                    Member member = roleToAdd.getGuild()
                    ...
                    if (member != null) {
                        ...
                        DiscordSRV.debug(
                            ...
                            "Couldn't find member for " + offlinePlayer.getName() + " in " + roleToAdd.getGuild());
                    }
                } else {
                    DiscordSRV.debug(
                        Debug.ACCOUNT_LINKING,
                        "Couldn't find \"account linked\" role " + roleName
                            + " to add to "
                            + offlinePlayer.getName()
                            + "'s linked Discord account");
                }
            } catch (Throwable t) {
                DiscordSRV.debug(
                    Debug.ACCOUNT_LINKING,
                    "Couldn't add \"account linked\" role \"" + roleName
                        + "\" due to exception: "
        ...
        if (DiscordSRV.config()
            ...
            DiscordSRV.getPlugin()
                .getNicknameUpdater()
                .setNickname(DiscordUtil.getMemberById(discordId), offlinePlayer);
        }
    }

    protected void beforeUnlink(UUID uuid, String discordId) {
        if (DiscordSRV.getPlugin()
            .isGroupRoleSynchronizationEnabled()) {
            DiscordSRV.getPlugin()
                .getGroupSynchronizationManager()
                .removeSynchronizables(Bukkit.getOfflinePlayer(uuid));
        } else {
            try {
                ...
                if (role != null) {
                    ...
                    if (member != null) {
                        ...
                        DiscordSRV
                            .debug(Debug.ACCOUNT_LINKING, "Couldn't remove \"linked\" role from null member: " + uuid);
                    }
                } else {
                ...
                DiscordSRV.debug(
                    Debug.ACCOUNT_LINKING,
                    "Failed to remove \"linked\" role from [" + uuid
                        + ":"
                        + discordId
                        + "] during unlink: "
                        + ExceptionUtils.getMessage(t));
    ...
    protected void afterUnlink(UUID uuid, String discordId) {
        ...
        User user = DiscordUtil.getUserById(discordId);
        for (String command : DiscordSRV.config()
            .getStringList("MinecraftDiscordAccountUnlinkedConsoleCommands")) {
            command = command
                .replace("%minecraftplayername%", PrettyUtil.beautifyUsername(offlinePlayer, "[Unknown player]", false))
        ...
        if (member != null && DiscordSRV.config()
            .getBoolean("NicknameSynchronizationEnabled")) {
            if (member.getGuild()
                .getSelfMember()
                .canInteract(member)) {
                ...
                DiscordSRV.debug(
                    Debug.ACCOUNT_LINKING,
                    "Can't remove nickname from " + member + ", bot is lower in hierarchy");
            }
        }
        ...
        if (player != null) {
            DiscordSRV.getPlugin()
                .getRequireLinkModule()
                .noticePlayerUnlink(player);
        }
    }
