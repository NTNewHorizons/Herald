...
public class JsonFileAccountLinkManager extends AbstractFileAccountLinkManager {
    ...
    void load() throws IOException {
        ...
        try {
            jsonObject = DiscordSRV.getPlugin()
                .getGson()
                .fromJson(fileContent, JsonObject.class);
        } catch (Throwable t) {
            if (!(t instanceof MalformedJsonException) && !(t instanceof JsonSyntaxException) || !t.getMessage()
                    ...
                    try {
                        ...
                        DiscordSRV.warning(
                            "Failed to load " + file.getName()
                                + " file. It's extremely recommended to delete your "
                                + file.getName()
                                + " file.");
                    }
                }
