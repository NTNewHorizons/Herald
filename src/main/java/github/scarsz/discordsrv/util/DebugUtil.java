...
public class DebugUtil {
    ...
    public static String run(String requester, int aesBits) {
        ...
        try {
            ...
            Runnable addDebugInfo = () -> files
                .add(fileMap("debug-info.txt", "Potential issues in the installation", debugInformation));
            ...
            files
                .add(
                    fileMap(
                        ...
                        String.join(
                            "\n",
                            new String[] { "Version information:", "   plugin version: " + DiscordSRV.getPlugin(),
                                "   config version: " + DiscordSRV.config()
                                ...
                                "   build origin: " + ManifestUtil.getManifestValue("Build-Origin"), "Plugin status:",
                                "   jda status: " + (DiscordUtil.getJda() != null && DiscordUtil.getJda()
                                    .getGatewayPing() != -1 ? DiscordUtil.getJda()
                                        .getStatus()
                                        .name() + " / "
                                        + DiscordUtil.getJda()
                                            .getGatewayPing()
                                        + "ms" : "build not finished"),
                                "   channels: " + DiscordSRV.getPlugin()
                                    .getChannels(),
                                "   console channel: " + DiscordSRV.getPlugin()
                                    .getConsoleChannel(),
                                "   main chat channel: " + DiscordSRV.getPlugin()
                                    .getMainChatChannel()
                                    + " -> "
                                    + DiscordSRV.getPlugin()
                                        .getMainTextChannel(),
                                "   main guild: " + DiscordSRV.getPlugin()
                                    .getMainGuild(),
                                "Environmental variables:", "   discord main guild roles: " + (DiscordSRV.getPlugin()
                                    .getMainGuild() == null ? "invalid main guild"
                                        : DiscordSRV.getPlugin()
                                            .getMainGuild()
                                            .getRoles()
                                            .stream()
                                ...
                                "   discord server owner: " + (DiscordSRV.getPlugin()
                                    .getMainGuild() == null ? "invalid main guild"
                                        : DiscordSRV.getPlugin()
                                            .getMainGuild()
                                            .getOwner()),
                                ...
                                "   /discord command executor: " + (Bukkit.getServer()
                                    .getPluginCommand("discord") != null ? Bukkit.getServer()
                                        .getPluginCommand("discord")
                                        .getPlugin() : ""),
                                "   hooked plugins: " + DiscordSRV.getPlugin()
                                    .getPluginHooks()
                                    .stream()
                                    .map(PluginHook::getPlugin)
                                    .filter(Objects::nonNull)
                                    .map(Object::toString)
                                    .collect(Collectors.joining(", ")),
                                "Threads:", "   channel topic updater -> alive: " + (DiscordSRV.getPlugin()
                                    .getChannelTopicUpdater() != null && DiscordSRV.getPlugin()
                                        .getChannelTopicUpdater()
                                        .isAlive()),
                                "   server watchdog -> alive: " + (DiscordSRV.getPlugin()
                                    .getServerWatchdog() != null && DiscordSRV.getPlugin()
                                        .getServerWatchdog()
                                        .isAlive()),
                                "   nickname updater -> alive: " + (DiscordSRV.getPlugin()
                                    .getNicknameUpdater() != null && DiscordSRV.getPlugin()
                                        .getNicknameUpdater()
                                        .isAlive()),
                                "   presence updater -> alive: " + (DiscordSRV.getPlugin()
                                    .getPresenceUpdater() != null && DiscordSRV.getPlugin()
                                        .getPresenceUpdater()
                                        .isAlive()),
                                "Guilds:" + listGuilds() })));
            files.add(
                fileMap(
                    "relevant-lines-from-server.log",
                    "lines from the server console containing \"discordsrv\"",
                    getRelevantLinesFromServerLog()));
            files.add(
                fileMap(
                    "config.yml",
                    "raw plugins/DiscordSRV/config.yml",
            ...
            files.add(
                fileMap(
                    "messages.yml",
                    "raw plugins/DiscordSRV/messages.yml",
                    FileUtils.readFileToString(
                        DiscordSRV.getPlugin()
                            .getMessagesFile(),
                        StandardCharsets.UTF_8)));
            files.add(
                fileMap(
                    "voice.yml",
                    "raw plugins/DiscordSRV/voice.yml",
                    FileUtils.readFileToString(
                        DiscordSRV.getPlugin()
                            .getVoiceFile(),
                        StandardCharsets.UTF_8)));
            files.add(
                fileMap(
                    "linking.yml",
                    "raw plugins/DiscordSRV/linking.yml",
                    FileUtils.readFileToString(
                        DiscordSRV.getPlugin()
                            .getLinkingFile(),
                        StandardCharsets.UTF_8)));
            files.add(
                fileMap(
                    "synchronization.yml",
                    "raw plugins/DiscordSRV/synchronization.yml",
                    FileUtils.readFileToString(
                        DiscordSRV.getPlugin()
            ...
            files.add(
                fileMap(
                    "alerts.yml",
                    "raw plugins/DiscordSRV/alerts.yml",
                    FileUtils.readFileToString(
                        DiscordSRV.getPlugin()
                            .getAlertsFile(),
                        StandardCharsets.UTF_8)));
            ...
            files.add(
                fileMap(
                    "registered-listeners.txt",
                    "list of registered listeners for Bukkit events DiscordSRV uses",
                    getRegisteredListeners()));
            files.add(fileMap("permissions.txt", null, getPermissions()));
            files.add(
                fileMap(
                    "threads.txt",
                    "Threads with DiscordSRV in the name or that have trace elements with DiscordSRV's classes",
                    getThreads()));
            files.add(fileMap("system-info.txt", null, getSystemInfo()));
            if (noIssues) addDebugInfo.run();
        } catch (Exception e) {
            DiscordSRV.error(e);
            return "Failed to collect debug information: " + e.getMessage()
    ...
    private static String listGuilds() {
        ...
        String list = "";
        for (Guild server : DiscordUtil.getJda()
            .getGuilds()) {
            list += "\n   " + server + ":  [";
    ...
    private static String getActiveConfig() {
        try {
            Dynamic activeConfig = DiscordSRV.config()
                .getProvider("config")
                .getValues();
            StringBuilder stringBuilder = new StringBuilder(500);
            Iterator<Dynamic> iterator = activeConfig.allChildren()
                .iterator();
                ...
                if (child.allChildren()
                    ...
                    stringBuilder.append(
                        child.key()
                            .asObject())
                        .append(": ")
                        .append(child.asObject());
                } else {
                    StringJoiner childJoiner = new StringJoiner(", ");

                    Iterator<Dynamic> childIterator = child.allChildren()
                        .iterator();
    ...
    private static String getInstalledPlaceholderApiExpansions() {
        ...
        File[] extensionFiles = new File(
            DiscordSRV.getPlugin()
                .getDataFolder()
                .getParentFile(),
            "PlaceholderAPI/expansions").listFiles();
        if (extensionFiles == null) return "PlaceholderAPI/expansions is not directory/IO error";
        return Arrays.stream(extensionFiles)
            .map(File::getName)
            .collect(Collectors.joining(", "));
    }
    ...
    private static String getServerInfo() {
        ...
        List<String> plugins = Arrays.stream(
        ...
        output.add(
            "server players: " + PlayerUtil.getOnlinePlayers()
    ...
    private static String getLoggerInfo() {
        ...
        try {
            ...
            output.add(
                "Log level: " + config.getConfiguration()
            ...
            for (Map.Entry<String, org.apache.logging.log4j.core.Appender> entry : rootLogger.getAppenders()
                .entrySet()) {
                appenders.add(
                    entry.getKey() + ": "
                        + entry.getValue()
    ...
    private static String getDebugInformation() {
        ...
        if (DiscordUtil.getJda() == null) {
            if (DiscordSRV.invalidBotToken
                || DiscordDisconnectListener.mostRecentCloseCode == CloseCode.AUTHENTICATION_FAILED) {
        ...
        if (DiscordUtil.getJda() != null) {
            ...
            for (Map.Entry<String, String> entry : DiscordSRV.getPlugin()
                .getChannels()
                .entrySet()) {
                TextChannel textChannel = DiscordUtil.getTextChannelById(entry.getValue());
                if (textChannel == null || !textChannel.getId()
                    .equals(entry.getValue())) {
                    messages.add(
                        new Message(Message.Type.INVALID_CHANNEL, "{" + entry.getKey() + ":" + entry.getValue() + "}"));
                    continue;
                }

                String configName = entry.getKey();
                String discordName = textChannel.getName();
                // contains non-alphanumeric & -whitespace characters (not a-z, 0-9 or whitespaces), "mc", "minecraft"
                // or "chat" or is "global"
                if (configName.equals(discordName) && (!configName.replaceAll("[\\w\\d\\s]", "")
                    .isEmpty() || configName.contains("mc")
                    || configName.contains("minecraft")
                    || configName.contains("chat")) && !configName.equals("global")) {
                    messages.add(new Message(Message.Type.SAME_CHANNEL_NAME, entry.getKey()));
                }
            }
        }

        String consoleChannelId = DiscordSRV.config()
            .getString("DiscordConsoleChannelId");
        ...
        String roleName = DiscordSRV.config()
            .getStringElse("MinecraftDiscordAccountLinkedRoleNameToAddUserTo", null);
        if (DiscordUtil.getJda() != null && roleName != null) {
            try {
                Role role = DiscordUtil.resolveRole(roleName);
                if (role != null && DiscordSRV.getPlugin()
                    .getGroupSynchronizables()
                    .values()
                    .stream()
                    .anyMatch(roleId -> roleId.equals(role.getId()))) {
        ...
        if (PluginUtil.pluginHookIsEnabled("TownyChat")) {
            try {
                ...
                if (mainChannelName != null && !DiscordSRV.getPlugin()
                    .getChannels()
                    .containsKey(mainChannelName)) {
        ...
        if (!DiscordSRV.config()
            .getBooleanElse("RespectChatPlugins", true)) {
        ...
        if (messages.isEmpty()) {
            ...
            messages.stream()
                .sorted((one, two) -> Boolean.compare(one.isWarning(), two.isWarning()))
                .forEach(
                    message -> stringBuilder.append(message.isWarning() ? "[Warn] " : "[Error] ")
        ...
        for (Class<?> listenedClass : listenedClasses) {
            try {
                ...
                List<RegisteredListener> registeredListeners = Arrays.stream(handlerList.getRegisteredListeners())
                    .filter(
                        registeredListener -> !registeredListener.getPlugin()
                            .getName()
                            .equalsIgnoreCase("DiscordSRV"))
                ...
                if (registeredListeners.isEmpty()) {
                    ...
                    output.add(
                        "Registered " + (listenedClass.isAnnotationPresent(Deprecated.class) ? "(DEPRECATED) " : "")
                            + listenedClass.getSimpleName()
                ...
                output.add(
                    "Error with " + listenedClass.getSimpleName()
                        + ": "
                        + e.getClass()
                            .getName()
                        + ": "
    ...
    private static String getPermissions() {
        ...
        TextChannel consoleChannel = DiscordSRV.getPlugin()
            .getConsoleChannel();
        ...
        DiscordSRV.getPlugin()
            .getChannels()
            .forEach((channel, textChannelId) -> {
                TextChannel textChannel = StringUtils.isNotBlank(textChannelId) ? DiscordSRV.getPlugin()
                    .getJda()
                    .getTextChannelById(textChannelId) : null;
                if (textChannel != null) {
    ...
    private static String getThreads() {
        ...
        for (Map.Entry<Thread, StackTraceElement[]> entry : stackTraces.entrySet()) {
            ...
            if ((threadName.contains("DiscordSRV") || Arrays.stream(traceElements)
                .anyMatch(
                    trace -> trace.getClassName()
                        .startsWith("github.scarsz.discordsrv")))
                && alreadyLoggedThreads.add(thread)) {
                stringBuilder.append(threadName)
                    .append(":\n")
                    .append(PrettyUtil.beautify(traceElements))
                    .append("\n");
            }
        }

        Thread serverThread = stackTraces.keySet()
            .stream()
            .filter(
                thread -> thread.getName()
                    .equals("Server thread"))
            .findAny()
            .orElse(null);
        ...
        for (Thread thread : stackTraces.keySet()) {
            if (alreadyLoggedThreads.add(thread)) {
                Plugin plugin = null;
                try {
                    plugin = SchedulerUtil.isFolia() ? null // not implemented on folia
                        : Bukkit.getScheduler()
                            .getActiveWorkers()
                            .stream()
        ...
        try {
            ...
            for (BukkitTask task : scheduler.getPendingTasks()) {
                scheduledTaskCounts.computeIfAbsent(task.getOwner(), key -> new AtomicInteger())
                    .incrementAndGet();
            }
            for (BukkitWorker activeWorker : scheduler.getActiveWorkers()) {
                runningTaskCounts.computeIfAbsent(activeWorker.getOwner(), key -> new AtomicInteger())
                    .incrementAndGet();
            }

            stringBuilder.append("\nScheduled tasks:\n");
            scheduledTaskCounts.forEach(
                (pl, in) -> stringBuilder.append(pl.getName())
                    .append(": ")
                    .append(in.get())
                    .append('\n'));

            stringBuilder.append("\nActive workers:\n");
            runningTaskCounts.forEach(
                (pl, in) -> stringBuilder.append(pl.getName())
                    .append(": ")
                    .append(in.get())
                    .append('\n'));
        } catch (Throwable t) {
            stringBuilder.append("\nFailed to get scheduler information: ")
                .append(t);
        }
    ...
    private static String getSystemInfo() {
        ...
        output.add(
            "Available processors (cores): " + Runtime.getRuntime()
                .availableProcessors());
        output.add("");

        // memory
        output.add(
            "Free memory for JVM (MB): " + Runtime.getRuntime()
                .freeMemory() / 1024
                / 1024);
        output.add(
            "Maximum memory for JVM (MB): " + (Runtime.getRuntime()
                .maxMemory() == Long.MAX_VALUE ? "no limit"
                    : Runtime.getRuntime()
                        .maxMemory() / 1024
                        / 1024));
        output.add(
            "Total memory available for JVM (MB): " + Runtime.getRuntime()
                .totalMemory() / 1024
                / 1024);
        output.add("");

        // drive space
        File serverRoot = DiscordSRV.getPlugin()
            .getDataFolder()
            .getAbsoluteFile()
            .getParentFile()
            .getParentFile();
        ...
        // java version
        Map<String, String> systemProperties = ManagementFactory.getRuntimeMXBean()
            .getSystemProperties();
        output.add("Java version: " + systemProperties.get("java.version"));
        output
            .add("Java vendor: " + systemProperties.get("java.vendor") + " " + systemProperties.get("java.vendor.url"));
    ...
    private static String uploadReport(List<Map<String, String>> files, int aesBits, String requester) {
        ...
        files.forEach(map -> {
            ...
            if (StringUtils.isNotBlank(content)) {
                ...
                for (String option : DebugUtil.SENSITIVE_OPTIONS) {
                    String value = DiscordSRV.config()
                        .getString(option);
        ...
        final ThreadFactory threadFactory = new ThreadFactoryBuilder().setNameFormat("DiscordSRV - Debug Report Upload")
            .build();
        ...
        try {
            ...
            if (e instanceof ExecutionException && e.getCause()
                .getMessage()
                .toLowerCase()
                .contains("illegal key size")) {
                return "ERROR/" + e.getCause()
                    .getMessage() + ". Try using /discordsrv debug 128";
            }

            File debugFolder = DiscordSRV.getPlugin()
                .getDebugFolder();
            ...
            return "GENERATED TO FILE/Failed to upload to bin.scarsz.me, placed into plugins/DiscordSRV/debug/"
                + debugName
                + ". Caused by "
                + (e instanceof ExecutionException ? e.getCause()
    ...
    private static String uploadToBin(String binHost, int aesBits, List<Map<String, String>> files,
        String description) {
        ...
        for (Map<String, String> file : files) {
            ...
            encryptedFile.entrySet()
                .removeIf(entry -> StringUtils.isBlank(entry.getValue()));
    ...
    public static class Message {
        ...
        public enum Type {
            ...
            // Errors
            RESPECT_CHAT_PLUGINS(false,
                "You have RespectChatPlugins set to false. This means DiscordSRV will completely ignore "
            ...
            PLUGIN_RELOADED(false,
                "Plugin has been initialized more than once (aka \"reloading\"). You will not receive support in this state."),
            ...
            CONSOLE_AND_CHAT_SAME_CHANNEL(false,
                LangUtil.InternalMessage.CONSOLE_CHANNEL_ASSIGNED_TO_LINKED_CHANNEL.getDefinitions()
                    .get(Language.EN)),
            NOT_IN_ANY_SERVERS(false, LangUtil.InternalMessage.BOT_NOT_IN_ANY_SERVERS.getDefinitions()
                .get(Language.EN)),
            ...
            DISALLOWED_INTENTS(false,
                "Disallowed intents (Make sure you followed all installation instructions), not connected to Discord."),
            ...
            UPDATE_AVAILABLE(false,
                "Update available. Download: https://get.discordsrv.com / https://snapshot.discordsrv.com"),
            LINKED_ROLE_GROUP_SYNC(false,
                "Cannot have the role in MinecraftDiscordAccountLinkedRoleNameToAddUserTo as a role in GroupRoleSynchronizationGroupsAndRolesToSync");
