...
public class DiscordChatChannelCommandFeedbackForwarder {
    ...
    public DiscordChatChannelCommandFeedbackForwarder(GuildMessageReceivedEvent event) {
        ...
        if (DiscordSRV.config()
            .getInt("DiscordChatChannelConsoleCommandExpiration") > 0 && DiscordSRV.config()
                .getBoolean("DiscordChatChannelConsoleCommandExpirationDeleteRequest")) {
            SchedulerUtil.runTaskLaterAsynchronously(
                ...
                DiscordSRV.config()
                    .getInt("DiscordChatChannelConsoleCommandExpiration") * 20L); // Seconds to ticks
        }
    }

    public void send(String message) {
        if (this.bufferCollecting) { // If the buffer has started collecting messages, we should just add this one to
                                     // it.
            if (DiscordUtil.escapeMarkdown(this.messageBuffer + "\n" + message)
                .length() > 1998) { // If the message will be too long (allowing for markdown escaping and the newline)
                // Send the message, then clear the buffer and add this message to the empty buffer
                DiscordUtil.sendMessage(
                    event.getChannel(),
                    DiscordUtil.escapeMarkdown(this.messageBuffer.toString()),
                    DiscordSRV.config()
                        .getInt("DiscordChatChannelConsoleCommandExpiration") * 1000);
