...
public class PluginUtil {
    ...
    public static void unloadPlugin(Plugin plugin) {
        ...
        try {
            Field pluginsField = Bukkit.getPluginManager()
                .getClass()
                .getDeclaredField("plugins");
            pluginsField.setAccessible(true);
            plugins = (List<Plugin>) pluginsField.get(pluginManager);

            Field lookupNamesField = Bukkit.getPluginManager()
                .getClass()
            ...
            try {
                Field listenersField = Bukkit.getPluginManager()
                    .getClass()
            ...
            Field commandMapField = Bukkit.getPluginManager()
                .getClass()
        ...
        if (commandMap != null) {
            for (Iterator<Map.Entry<String, Command>> it = commands.entrySet()
                .iterator(); it.hasNext();) {
        ...
        ClassLoader cl = plugin.getClass()
            .getClassLoader();
        if (cl instanceof URLClassLoader) {
            try {
                ((URLClassLoader) cl).close();
            } catch (IOException ex) {
                Logger.getLogger(PluginUtil.class.getName())
    ...
    public static boolean pluginHookIsEnabled(String pluginName) {
        ...
        for (String pluginHookName : DiscordSRV.config()
            .getStringList("DisabledPluginHooks")) {
            if (pluginName.toLowerCase()
                .startsWith(pluginHookName.toLowerCase())) {
                enabled = false;
                break;
            }
        }
    ...
    public static Plugin getPlugin(String pluginName) {
        for (Plugin plugin : Bukkit.getPluginManager()
            .getPlugins())
            if (plugin.getName()
                .equalsIgnoreCase(pluginName)) return plugin;
        return null;
    }
